...
def dnn_training(
    ...
    model_type: str | None = "dnn",
    dataset_path: str | None = None,
    score_thold: float = 0.5,
):
    """
    DNN Model training pipeline.

    This pipeline loads the data, trains a Deep Neural Network using PyTorch Lightning,
    ...
    Args:
        ...
        model_type: The type of model to train (kept for config compatibility).
    ...
    # dnn_trainer expects a single target string
    target_col = targets[0] if targets and len(targets) > 0 else "target"

    model = dnn_trainer(dataset_trn=dataset_trn, target=target_col)

    # We use the classifier evaluator as the DNN model is a classifier
    score = classifier_model_evaluator(
        model=model,
    ...
    model_promoter(model_class="classifier", score=score, score_thold=score_thold)
